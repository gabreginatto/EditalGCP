...
Usage:
    python dynamic_handler_SANEAGO.py --url URL --output-dir DIR [--timeout SECONDS]
"""
...
log_file = f"dynamic_handler_saneago_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("SANEAGOHandler")

# Define paths
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
STAGEHAND_DIR = os.path.join(SCRIPT_DIR, "stagehand")
HANDLER_SCRIPT = os.path.join(STAGEHAND_DIR, "handler_saneago.js")
...
def setup_output_dirs(base_dir):
    ...
    output_dirs = {
        ...
        'stagehand_downloads': os.path.join(STAGEHAND_DIR, 'downloads')
    }
...
def run_stagehand_handler(output_dir, timeout):
    ...
    try:
        ...
        logger.info(f"Starting SANEAGO Stagehand handler (timeout: {timeout}s)")
        ...
        if process.returncode == 0:
            ...
            zip_pattern = os.path.join(output_dirs['stagehand_downloads'], "Saneago_*.zip")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SANEAGO Download Handler")
